...
bool Continuous_Property::measure_residuals( Basic_input &input )
{
	...
	for (int j = 0; j < (int)input.interface.size(); j++ ){
		...
		input.interface[j].setResidual(abs(input.interface[j].scalar_field() - input.interface[j].level()));
	}
	// planar points
...
{
	for (int j = 0; j < (int)b_input.interface.size(); j++) equality_values.push_back(b_input.interface[j].level());
