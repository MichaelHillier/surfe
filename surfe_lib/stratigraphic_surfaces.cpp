...
bool Stratigraphic_Surfaces::_get_increment_pairs()
{
	...
	for (int j = 0; j < _n_sequenced_interface_pairs; j++ ){
		_increment_pairs[j].push_back(b_input.interface_test_points[j]);
		_increment_pairs[j].push_back(b_input.interface_test_points[j + 1]);
	}
	// 2) lithostratigraphic inequalities 
	_n_sequenced_inequality_pairs = 0;
	for (int j = 0; j < (int)b_input.inequality.size(); j++ ){
		std::vector < std::vector <Point> > strati_seq_ine_pairs = _get_lithostratigraphic_increment_pairs_for_inequality_point(b_input.inequality[j]);
	...
	for (int j = 0; j < (int)b_input.interface_point_lists.size(); j++ ) _n_interface_pairs += ((int)b_input.interface_point_lists[j].size() - 1);
	for (int j = 0; j < (int)b_input.interface_point_lists.size(); j++ ){
		for (int k = 0; k < ((int)b_input.interface_point_lists[j].size() - 1); k++ ){
			std::vector<Point> interface_incr_p;
			interface_incr_p.push_back(b_input.interface_point_lists[j][0]);
			interface_incr_p.push_back(b_input.interface_point_lists[j][k + 1]);
...
std::vector < std::vector < Point > > Stratigraphic_Surfaces::_get_lithostratigraphic_increment_pairs_for_inequality_point( const Inequality &ie_pt )
{
	...
	for (int j = 0; j < (int)b_input.interface_test_points.size(); j++ ) horizon_levels.push_back(b_input.interface_test_points[j].level());
	...
	if (level_above != NULL)
	{
		...
		for (int j = 0; j < (int)b_input.interface_test_points.size(); j++ ){
			if (b_input.interface_test_points[j].level() == level_above)
			{
				strati_incr_p[n_pair].push_back(b_input.interface_test_points[j]);
	...
	if (level_below != NULL)
	{
		strati_incr_p.resize(n_pair + 1);
		for (int j = 0; j < (int)b_input.interface_test_points.size(); j++ ){
			if (b_input.interface_test_points[j].level() == level_below)
			{
				strati_incr_p[n_pair].push_back(ie_pt);
				strati_incr_p[n_pair].push_back(b_input.interface_test_points[j]);
				break;
...
bool Stratigraphic_Surfaces::get_interpolation_matrix( std::vector< std::vector <double> > &interpolation_matrix )
{
	...
	for (int j = 0; j < (int)_increment_pairs.size(); j++ ){
		...
		for (int k = 0; k < n_p; k++ ){
			kernel->set_points(_increment_pairs[j][0],b_input.planar[k]);
			double v1x = kernel->basis_pt_planar_x();
			double v1y = kernel->basis_pt_planar_y();
			double v1z = kernel->basis_pt_planar_z();
			kernel->set_points(_increment_pairs[j][1],b_input.planar[k]);
		...
		for (int k = 0; k < n_t; k++ ){
			kernel->set_points(_increment_pairs[j][0],b_input.tangent[k]);
			double v1 = kernel->basis_pt_tangent();
			kernel->set_points(_increment_pairs[j][1],b_input.tangent[k]);
	...
	for (int j = 0; j < n_p; j++ ){
		...
		for (int k = 0; k < (int)_increment_pairs.size();k++ ){
			kernel->set_points(b_input.planar[j],_increment_pairs[k][0]);
			double v1x = kernel->basis_planar_x_pt();
			double v1y = kernel->basis_planar_y_pt();
			double v1z = kernel->basis_planar_z_pt();
			kernel->set_points(b_input.planar[j],_increment_pairs[k][1]);
	...
	for (int j = 0; j < n_t; j++ ){
		...
		for (int k = 0; k < (int)_increment_pairs.size();k++ ){
			kernel->set_points(b_input.tangent[j],_increment_pairs[k][0]);
			double v1 = kernel->basis_tangent_pt();
			kernel->set_points(b_input.tangent[j],_increment_pairs[k][1]);
