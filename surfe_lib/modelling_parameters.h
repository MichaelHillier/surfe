...
struct SURFE_LIB_EXPORT model_parameters{
	...
	model_parameters() : model_type(Parameter_Types::Single_surface), min_stratigraphic_thickness(0),
		...
		advanced_parameters(false), model_global_anisotropy(false), use_greedy(false), use_restricted_range(false), use_regression_smoothing(false), interface_uncertainty(0), angular_uncertainty(0) {}
};
