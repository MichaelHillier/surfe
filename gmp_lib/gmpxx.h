...
#define __GMPQ_DEN_DBL_LIMBS                                                   \
  __GMPXX_BITS_TO_LIMBS(DBL_MANT_DIG + 1 - DBL_MIN_EXP) + 1
...
inline void __gmp_binary_plus::eval(mpq_ptr q, mpq_srcptr r,
                                    signed long int l) {
  if (l >= 0)
    eval(q, r, static_cast<unsigned long>(l));
  else
...
struct __gmp_binary_greater {
  template <class T, class U> static inline bool eval(T t, U u) {
...
template <class T, class U> struct common_type<__gmp_expr<T, U>> {
...
#define __GMP_DEFINE_BINARY_FUNCTION_UI(fun, eval_fun)                         \
  ...
  fun(const __gmp_expr<T, U> &expr, mp_bitcnt_t l) {                           \
    return __gmp_expr<                                                         \
        T, __gmp_binary_expr<__gmp_expr<T, U>, mp_bitcnt_t, eval_fun>>(expr,   \
                                                                       l);     \
  }
...
#define __GMPNS_DEFINE_BINARY_TYPE_FUNCTION(type, fun, eval_fun, type2)        \
  __GMPNN_DEFINE_BINARY_TYPE_FUNCTION(type, fun, eval_fun, type2,              \
                                      signed long int)
...
#define __GMPNU_DEFINE_COMPOUND_OPERATOR(type, fun, eval_fun, type2)           \
  __GMPNN_DEFINE_COMPOUND_OPERATOR(type, fun, eval_fun, type2,                 \
                                   unsigned long int)
...
#define __GMP_DEFINE_COMPOUND_OPERATOR_UI(type, fun, eval_fun)                 \
  ...
  inline type##_class &type##_class::fun(mp_bitcnt_t l) {                      \
    __gmp_set_expr(                                                            \
        mp,                                                                    \
        __gmp_expr<type##_t,                                                   \
                   __gmp_binary_expr<type##_class, mp_bitcnt_t, eval_fun>>(    \
            *this, l));                                                        \
    return *this;                                                              \
  }
...
typedef void __gmp_randinit_lc_2exp_t(gmp_randstate_t, mpz_srcptr,
...
public:
  ...
  gmp_randclass(__gmp_randinit_lc_2exp_t *f, mpz_class z, unsigned long int l1,
                mp_bitcnt_t l2) {
